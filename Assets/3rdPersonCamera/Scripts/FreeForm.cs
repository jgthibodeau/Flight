    ...
    public class FreeForm : MonoBehaviour {
        ...
        public void Start() {
			...
			if (platform.Contains ("windows") || platform.Contains ("linux")) {
				rightAxisXName = "Horizontal Right";
				rightAxisYName = "Vertical Right";
			} else {
				rightAxisXName = "Horizontal Right";
				rightAxisYName = "Vertical Right";
			}
        ...
        public void Update() {
			if (cameraEnabled) {
				...
				if (mouseEnabled && ((cameraMode == CameraMode.Hold && mouse0 || mouse1) || cameraMode == CameraMode.Always)) {
				...
				if (yAngle > 0) {
					if (angle + yAngle >= maxYAngle) {
//						yAngle = Vector3.Angle (transform.forward, upVector) - 180;
						yAngle = maxYAngle - angle;

						if (yAngle < 0)
							yAngle = 0;
					}
				} else {
					if (angle + yAngle <= minYAngle) {
//						yAngle = Vector3.Angle (transform.forward, downVector) - 180;
//						angle + yAngle = 0f;
						yAngle = minYAngle - angle;
					}
				}
//				if (angle <= yAngleLimitMin && yAngle < 0) {
////					yAngle = 0;
//					yAngle = Mathf.Clamp (yAngle, , 0);
//				} else if (angle >= yAngleLimitMax && yAngle > 0) {
////					yAngle = 0;
