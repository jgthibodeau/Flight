...
Shader "Custom/CloudSky"
{
    ...
    SubShader
    {
		...
		//"LightMode"="ForwardBase" "RenderType"="Transparent" "Queue"="Transparent" 
        ...
        Pass
        {
            ...
            #include "Assets/Resources/Clouds/Shaders/CloudDebug.cginc"
            ...
            float4 colA;
            float4 colB;
			uniform float4 LSky_Color;
			uniform float4 LSky_Dir;
			uniform float4 LSky_CloudColor;
			uniform float4 LSky_CloudIntensity;
            ...
            float lightmarch(float3 p) {
                float3 dirToLight = LSky_Dir.xyz;
...
//                float focusedEyeCos = pow(saturate(cosAngle), params.x);
 //               float sun = saturate(hg(focusedEyeCos, .9995)) * transmittance;
                ...
                float3 originalCol = LSky_CloudColor;


                float3 cloudCol = lightEnergy * originalCol;
                float3 col = backgroundCol * transmittance + cloudCol;
   //             col = saturate(col) * (1-sun) + originalCol *sun;
                return float4(col,0);
            }
            ENDCG
        }
