...
public class WeatherMap : MonoBehaviour {
    ...
    void CreateTexture (ref RenderTexture texture, int resolution) {
        var format = UnityEngine.Experimental.Rendering.GraphicsFormat.R32G32B32A32_UInt;
        if (texture == null || !texture.IsCreated () || texture.width != resolution || texture.height != resolution || texture.graphicsFormat != format) {
            Debug.Log((texture == null) + " " + !texture.IsCreated() + " " + (texture.width != resolution) + " " + (texture.height != resolution) + " " + (texture.graphicsFormat != format));
            if (texture != null) {
                texture.Release ();
            }
