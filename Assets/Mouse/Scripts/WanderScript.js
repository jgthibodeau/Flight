...
function Start () {
	rayDist = GetComponent(CapsuleCollider).radius * 2; //set the rayDist for this animal | the rayDist is equal to twice the radius of the animal's CharacterController
...
function Update() {
	...
	distanceBetween = Vector3.Distance(transform.position, startPos);
	...
	//while timer is less than walking time, keep walking
		...
		if(timer < waitTime) {
			...
			if(distanceBetween < maxDistance || returningHome == true) { //if within maxDistance, or returingHome
				...
				raycastPoint = Vector3(transform.position.x, GetComponent(CapsuleCollider).height / 2, transform.position.z);
