...
public class QuadtreeNode {
    ...
    public QuadtreeNode(Vector3 _center, float _size, float _minSize) {
        Center = _center;
        Size = _size;
        minSize = _minSize;

        Bounds = new Bounds(center, new Vector3(size,minSize,size));
        UpdateChildrenCenters();
    }

    public QuadtreeNode(QuadtreeNode _other) {
        this.Center = _other.center;
        this.Size = _other.Size;
        this.minSize = _other.minSize;

        this.Bounds = new Bounds(center, new Vector3(size, minSize, size));
    }

    public bool Add(IOctreeObject _object) {

        if (!this.Bounds.Contains( new Vector3(_object.Position.x,0, _object.Position.z))) {
            return false;
        }
    ...
    public void SetChildren(QuadtreeNode[] _children) {
        children = _children;
    }
    ...
    public QuadtreeNode ShrinkIfPossible(float _initialSize) {
    ...
    private void Split() {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            children[i] = new QuadtreeNode(this.Center + new Vector3(xDir * _quarter, 0, zDir * _quarter), _newSize, minSize);
        }
    }
    ...
    private void Merge() {
        ...
        for (int i = 0; i < 4; i++) {
            QuadtreeNode curChild = children[i];
    ...
    private void UpdateChildrenCenters() {
        ...
        childrenCenters = new Vector3[4];
        childrenCenters[0] = Center + new Vector3(-quarter, 0, -quarter);
        childrenCenters[1] = Center + new Vector3(quarter, 0, -quarter);
        childrenCenters[2] = Center + new Vector3(-quarter, 0, quarter);
        childrenCenters[3] = Center + new Vector3(quarter, 0, quarter);
    }
